      ...
      {images.length > 0 && (
        <span className="mt-2 flex flex-wrap gap-2" data-slot="aui_embedded-images">
          {images.map((src, index) => (
            <ZoomableImage
              alt=""
              className="max-h-48 max-w-full rounded-[12px] border border-border/60 object-contain"
              draggable={false}
              key={`img-${index}`}
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  return (
    <ZoomableImage
      alt={label}
      className="max-h-32 max-w-48 rounded-[12px] border border-border/40 object-contain"
      draggable={false}
