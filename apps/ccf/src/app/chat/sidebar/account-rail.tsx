import { useNavigate } from 'react-router-dom'

import { ChevronDown } from '@/lib/icons'
import { profileColor, profileColorSoft } from '@/lib/profile-color'
import { cn } from '@/lib/utils'

import { SETTINGS_ROUTE } from '../../routes'

// Placeholder account row for the sidebar footer — Hermes' backend has no
// user-account concept yet (OAuth here connects LLM *providers*, not an app
// account), so the name is static until CCF has a real account system to
// read from. Visually matches the target: avatar, name, chevron.
const ACCOUNT_NAME = 'Marty'

export function AccountRail() {
  const navigate = useNavigate()
  const hue = profileColor(ACCOUNT_NAME) ?? 'var(--ui-text-quaternary)'

  return (
    <button
      aria-label={ACCOUNT_NAME}
      className={cn(
        'flex w-full items-center gap-2 rounded-[8px] px-2 py-1.5 text-left transition-colors',
        'hover:bg-(--ui-control-hover-background)'
      )}
      onClick={() => navigate(SETTINGS_ROUTE)}
      type="button"
    >
      <span
        aria-hidden="true"
        className="grid size-5 shrink-0 place-items-center rounded-full text-[0.625rem] font-semibold uppercase leading-none"
        style={{ backgroundColor: profileColorSoft(hue, 28), color: hue }}
      >
        {ACCOUNT_NAME.charAt(0)}
      </span>
      <span className="min-w-0 flex-1 truncate text-[0.8125rem] font-medium text-foreground">{ACCOUNT_NAME}</span>
      <ChevronDown className="size-3.5 shrink-0 text-(--ui-text-tertiary)" />
    </button>
  )
}
