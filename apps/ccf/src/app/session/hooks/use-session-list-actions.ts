  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      if (refreshSessionsRequestRef.current === requestId) {
        ...
        // Signature-gate the swap (same pattern as cron/messaging below): a
        // refresh that returns content-identical rows must keep the previous
        // array identity, or every sidebar memo keyed on $sessions recomputes
        // and the whole list re-renders once per turn/broadcast for nothing.
        setSessions(prev => {
        ...
        // resolves via sessionByAnyId), signature-gated like the other sections.
