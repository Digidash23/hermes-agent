      ...
      const completedState = updateSessionState(sessionId, state => {
        ...
        if (streamId && prev.some(m => m.id === streamId)) {
          ...
          if (fallbackIndex >= 0) {
            ...
            // When `interimBoundaryPending` is set, the last assistant row IS the
            // just-sealed interim (a tool-call turn or a verify-on-stop candidate
            // — `message.interim` fires for BOTH, see tui_gateway
            // `_load_interim_assistant_messages`): it was appended immediately
            // before this flag was set, with nothing else able to land between
            // the two in this reducer flow. When the final completion is the SAME
            // turn's reply, settle it onto that interim instead of appending a
            // second bubble. Continuity, not exact equality: streaming can drop
            // characters and the final may add a trailing delta, so treat
            // prefix-either-way as the same message. (mergeFinalAssistantText, via
            // completeMessage, does the real text merge — replaces the interim's
